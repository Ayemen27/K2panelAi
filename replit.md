...
- `PORT` - رقم البورت الذي سيعمل عليه السيرفر (افتراضي: 5000)
  - للتغيير: عدّل القيمة في `.env.local` ثم أعد تشغيل السيرفر
  - مثال: `PORT=8080` للبورت 8080
...
- **20 نوفمبر 2025 - ليلاً**: 🔧 **تحديث شامل لإعدادات البورت (PORT)**
  ...
  - ✅ إنشاء سكريبتات wrapper (`scripts/start-dev.js` & `scripts/start-prod.js`) لقراءة PORT من `.env.local`
  - ✅ تحديث جميع الملفات البرمجية لاحترام متغير PORT:
    - `scripts/update-nextauth-url.js` - يقرأ PORT من البيئة
    - `scripts/test-auth.js` - يقرأ PORT من البيئة
    - `src/lib/env.ts` - getReplitUrl() يستخدم PORT
    - `src/lib/apollo-client.ts` - getGraphQLEndpoint() يستخدم PORT
  - ✅ تحديث `package.json` scripts لاستخدام السكريبتات الجديدة
  - ✅ تحديث `.env.example` لتوثيق المتغير
  - 🎯 **المرونة الكاملة**: جميع السكريبتات والملفات تحترم متغير PORT الآن
  - 📝 **الاستخدام**: فقط عدّل `PORT=XXXX` في `.env.local` وأعد تشغيل السيرفر
  - ✅ **اختبار ناجح**: السيرفر يعمل الآن على البورت 5000 من `.env.local`
